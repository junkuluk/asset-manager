...
if source_df.empty:
    ...
    report_df.loc[('수입', '수입 소계'), :] = report_df.loc['수입'].drop('수입 소계').sum()
    report_df.loc[('지출', '지출 소계'), :] = report_df.loc['지출'].drop('지출 소계').sum()
    report_df.loc[('투자', '투자 소계'), :] = report_df.loc['투자'].drop('투자 소계').sum()
    ...
    def apply_report_styles(df):
        ...
        def get_row_style(row):
            ...
            # 기본 배경색 설정
            color_map = {
                '수입': '#fffbe6', '지출': '#f8f9fa', '투자': '#e6f4ea'
            }
            total_color_map = {
                '수입': '#fff3cd', '지출': '#e9ecef', '투자': '#cce8d4'
            }
        ...
        # 스타일 적용
        styled = df.style.apply(get_row_style, axis=1) \
            .format("{:,.0f}", na_rep='-', subset=numeric_cols) \
            .set_properties(subset=numeric_cols, **{'text-align': 'right', 'width': '120px'}) \
            .set_table_styles([
    ...
    st.dataframe(styled_df, use_container_width=True, height=(len(final_df_to_style) + 1) * 35 + 3)
...
if asset_df.empty:
    ...
    styled_asset_df = asset_df.style.format("{:,.0f}원", na_rep='-') \
        .set_properties(**{'text-align': 'right'}) \
        .apply(lambda row: ['font-weight: bold; background-color: #f7f7f7' if row.name == '총 자산' else '' for _ in row],
               axis=1)

    st.dataframe(styled_asset_df, use_container_width=True, height=(len(asset_df) + 1) * 35 + 3)