...
st.markdown("은행 출금 내역 중 '지출'로 잘못 분류된 항목을 카드값 납부나 투자 이체 등으로 변경합니다.")
st.markdown("---")
...
if not candidate_df.empty:
    ...
    gridOptions = {
        "columnDefs": [
            ...
            {"field": "transaction_amount", "headerName": "금액", "type": "numericColumn",
    ...
    if selected_candidate is not None and not selected_candidate.empty:
        ...
        with col_form:
            with st.form("reclassify_form"):
                ...
                linked_account_name = st.selectbox("이 돈이 어디로 이체/투자되었나요?", options=list(all_accounts_map.keys()))

                submitted = st.form_submit_button("거래 성격 변경하기", use_container_width=True, type="primary")
                if submitted:
