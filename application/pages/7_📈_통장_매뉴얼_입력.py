...
st.set_page_config(layout="wide", page_title="통장 매뉴얼 입력")
st.title("📈 통장 매뉴얼 입력")
st.markdown("---")

# 투자 계좌 정보 로드
investment_df = get_bank_accounts()

income_categories = get_all_categories(category_type="INCOME")
category_name_to_id_map = {v: k for k, v in income_categories.items()}
...
if investment_df.empty:
    st.warning(
        "등록된 은행 자산이 없습니다. '기준정보 관리'에서 먼저 계좌를 추가해주세요."
    )
else:
    with st.form("transaction_form"):
        ...
        with col1:
            selected_asset_name = st.selectbox(
                "은행계좌", options=investment_df["name"]
            )
